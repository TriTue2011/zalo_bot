...
def serve_file_temporarily(file_path, duration=60):
        ...
        def do_GET(self):
            if self.path == f"/{encoded_filename}" or self.path == "/":
                try:
                    ...
                    _LOGGER.error(f"Error serving file: {str(e)}")
    ...
    def close_server():
        ...
        httpd.shutdown()
        httpd.server_close()
        _LOGGER.debug(f"Temporary server for {file_name} stopped")
    
    shutdown_thread = threading.Thread(target=close_server)
    shutdown_thread.daemon = True
    shutdown_thread.start()
    
    _LOGGER.debug(f"Serving {file_name} at {url} for {duration} seconds")
    return url
...
async def show_result_notification(hass, service_name, resp, error=None):
    try:
        ...
        _LOGGER.error("Error in show_result_notification: %s", e)
...
async def async_setup_entry(hass, entry):
    ...
    # New schemas for added services
    ...
    SERVICE_CREATE_GROUP_SCHEMA = vol.Schema({
        vol.Required("members"): str,  # Comma-separated list
    ...
    SERVICE_ADD_USER_TO_GROUP_SCHEMA = vol.Schema({
        ...
        vol.Required("member_id"): str,  # Comma-separated for multiple
        vol.Required("account_selection"): str,
    })
    ...
    SERVICE_SEND_IMAGES_TO_USER_SCHEMA = vol.Schema({
        vol.Required("image_paths"): str,  # Comma-separated URLs
        vol.Required("thread_id"): str,
        vol.Required("account_selection"): str,
    })
    ...
    # Full implementations for all new services to fix unused schemas

    async def async_get_logged_accounts_service(call):
        _LOGGER.debug("Called get_logged_accounts with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            resp = await hass.async_add_executor_job(
                lambda: session.get(f"{zalo_server}/api/accounts")
            )
            _LOGGER.info("Get logged accounts response: %s", resp.text)
            await show_result_notification(hass, "lấy danh sách tài khoản", resp)
        except Exception as e:
            _LOGGER.error(
                "Error in get_logged_accounts: %s", e
            )
    ...
    async def async_get_account_details_service(call):
        _LOGGER.debug("Called get_account_details with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            ...
            _LOGGER.info("Get account details response: %s", resp.text)
            await show_result_notification(hass, "lấy chi tiết tài khoản", resp)
        except Exception as e:
            _LOGGER.error("Error in get_account_details: %s", e)
            await show_result_notification(hass, "lấy chi tiết tài khoản", None, error=e)

    hass.services.async_register(DOMAIN, "get_account_details", async_get_account_details_service, schema=SERVICE_GET_ACCOUNT_DETAILS_SCHEMA)

    async def async_find_user_service(call):
        _LOGGER.debug("Called find_user with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
            ...
            _LOGGER.info("Find user response: %s", resp.text)
            await show_result_notification(hass, "tìm người dùng", resp)
        except Exception as e:
            _LOGGER.error("Error in find_user: %s", e)
    ...
    async def async_get_user_info_service(call):
        _LOGGER.debug("Called get_user_info with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
            ...
            _LOGGER.info("Get user info response: %s", resp.text)
            await show_result_notification(hass, "lấy thông tin người dùng", resp)
        except Exception as e:
            _LOGGER.error("Error in get_user_info: %s", e)
            await show_result_notification(hass, "lấy thông tin người dùng", None, error=e)

    hass.services.async_register(DOMAIN, "get_user_info", async_get_user_info_service, schema=SERVICE_GET_USER_INFO_SCHEMA)

    async def async_send_friend_request_service(call):
        _LOGGER.debug("Called send_friend_request with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
                "userId": call.data["user_id"],
            ...
            _LOGGER.info("Send friend request response: %s", resp.text)
            await show_result_notification(hass, "gửi lời mời kết bạn", resp)
        except Exception as e:
            _LOGGER.error("Error in send_friend_request: %s", e)
            await show_result_notification(hass, "gửi lời mời kết bạn", None, error=e)

    hass.services.async_register(DOMAIN, "send_friend_request", async_send_friend_request_service, schema=SERVICE_SEND_FRIEND_REQUEST_SCHEMA)

    async def async_create_group_service(call):
        _LOGGER.debug("Called create_group with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            ...
            _LOGGER.info("Create group response: %s", resp.text)
            await show_result_notification(hass, "tạo nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in create_group: %s", e)
    ...
    async def async_get_group_info_service(call):
        _LOGGER.debug("Called get_group_info with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            group_id = call.data.get("group_id", "")
            account_selection = call.data.get("account_selection", "") or "default"  # Default logic
            group_id_list = (
            ...
            _LOGGER.info("Get group info response: %s", resp.text)
            await show_result_notification(hass, "lấy thông tin nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in get_group_info: %s", e)
            await show_result_notification(hass, "lấy thông tin nhóm", None, error=e)

    hass.services.async_register(DOMAIN, "get_group_info", async_get_group_info_service, schema=SERVICE_GET_GROUP_INFO_SCHEMA)

    async def async_add_user_to_group_service(call):
        _LOGGER.debug("Called add_user_to_group with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            member_id_list = (
            ...
            _LOGGER.info("Add user to group response: %s", resp.text)
            await show_result_notification(hass, "thêm người dùng vào nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in add_user_to_group: %s", e)
            await show_result_notification(hass, "thêm người dùng vào nhóm", None, error=e)

    hass.services.async_register(DOMAIN, "add_user_to_group", async_add_user_to_group_service, schema=SERVICE_ADD_USER_TO_GROUP_SCHEMA)

    async def async_remove_user_from_group_service(call):
        _LOGGER.debug("Called remove_user_from_group with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            member_id_list = call.data["member_id"].split(",") if "," in call.data["member_id"] else call.data["member_id"]
            payload = {
                "groupId": call.data["group_id"],
                "memberId": member_id_list,
                "accountSelection": call.data["account_selection"]
            ...
            _LOGGER.info("Remove user from group response: %s", resp.text)
            await show_result_notification(hass, "xóa người dùng khỏi nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in remove_user_from_group: %s", e)
            await show_result_notification(hass, "xóa người dùng khỏi nhóm", None, error=e)

    hass.services.async_register(DOMAIN, "remove_user_from_group", async_remove_user_from_group_service, schema=SERVICE_REMOVE_USER_FROM_GROUP_SCHEMA)

    async def async_send_image_to_user_service(call):
        _LOGGER.debug("Called send_image_to_user with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            image_path = call.data["image_path"]
            ...
            _LOGGER.info("Send image to user response: %s", resp.text)
            await show_result_notification(hass, "gửi ảnh cho người dùng", resp)
        except Exception as e:
            _LOGGER.error("Error in send_image_to_user: %s", e)
            await show_result_notification(hass, "gửi ảnh cho người dùng", None, error=e)

    hass.services.async_register(DOMAIN, "send_image_to_user", async_send_image_to_user_service, schema=SERVICE_SEND_IMAGE_TO_USER_SCHEMA)

    async def async_send_image_to_group_service(call):
        _LOGGER.debug("Called send_image_to_group with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            image_path = call.data["image_path"]
            ...
            _LOGGER.info("Send image to group response: %s", resp.text)
            await show_result_notification(hass, "gửi ảnh cho nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in send_image_to_group: %s", e)
            await show_result_notification(hass, "gửi ảnh cho nhóm", None, error=e)

    hass.services.async_register(DOMAIN, "send_image_to_group", async_send_image_to_group_service, schema=SERVICE_SEND_IMAGE_TO_GROUP_SCHEMA)

    async def async_send_images_to_user_service(call):
        _LOGGER.debug("Called send_images_to_user with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            ...
            _LOGGER.info("Send images to user response: %s", resp.text)
            await show_result_notification(hass, "gửi nhiều ảnh cho người dùng", resp)
        except Exception as e:
            _LOGGER.error("Error in send_images_to_user: %s", e)
            await show_result_notification(hass, "gửi nhiều ảnh cho người dùng", None, error=e)

    hass.services.async_register(DOMAIN, "send_images_to_user", async_send_images_to_user_service, schema=SERVICE_SEND_IMAGES_TO_USER_SCHEMA)

    async def async_send_images_to_group_service(call):
        _LOGGER.debug("Called send_images_to_group with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            image_paths = call.data["image_paths"].split(",")
            processed_paths = []
            ...
            _LOGGER.info("Send images to group response: %s", resp.text)
            await show_result_notification(hass, "gửi nhiều ảnh cho nhóm", resp)
        except Exception as e:
            _LOGGER.error("Error in send_images_to_group: %s", e)
            await show_result_notification(hass, "gửi nhiều ảnh cho nhóm", None, error=e)

    hass.services.async_register(DOMAIN, "send_images_to_group", async_send_images_to_group_service, schema=SERVICE_SEND_IMAGES_TO_GROUP_SCHEMA)

    async def async_get_account_webhooks_service(call):
        _LOGGER.debug("Called get_account_webhooks with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            resp = await hass.async_add_executor_job(
                lambda: session.get(f"{zalo_server}/api/account-webhooks")
            )
            _LOGGER.info("Get account webhooks response: %s", resp.text)
            await show_result_notification(hass, "lấy danh sách webhook", resp)
        except Exception as e:
            _LOGGER.error("Error in get_account_webhooks: %s", e)
            await show_result_notification(hass, "lấy danh sách webhook", None, error=e)

    hass.services.async_register(DOMAIN, "get_account_webhooks", async_get_account_webhooks_service, schema=SERVICE_GET_ACCOUNT_WEBHOOKS_SCHEMA)

    async def async_get_account_webhook_service(call):
        _LOGGER.debug("Called get_account_webhook with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            own_id = call.data["own_id"]
            resp = await hass.async_add_executor_job(
            ...
            _LOGGER.info("Get account webhook response: %s", resp.text)
            await show_result_notification(hass, "lấy thông tin webhook", resp)
        except Exception as e:
            _LOGGER.error("Error in get_account_webhook: %s", e)
            await show_result_notification(hass, "lấy thông tin webhook", None, error=e)

    hass.services.async_register(DOMAIN, "get_account_webhook", async_get_account_webhook_service, schema=SERVICE_GET_ACCOUNT_WEBHOOK_SCHEMA)

    async def async_set_account_webhook_service(call):
        _LOGGER.debug("Called set_account_webhook with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
            ...
            _LOGGER.info("Set account webhook response: %s", resp.text)
            await show_result_notification(hass, "cài đặt webhook", resp)
        except Exception as e:
            _LOGGER.error("Error in set_account_webhook: %s", e)
            await show_result_notification(hass, "cài đặt webhook", None, error=e)

    hass.services.async_register(DOMAIN, "set_account_webhook", async_set_account_webhook_service, schema=SERVICE_SET_ACCOUNT_WEBHOOK_SCHEMA)

    async def async_delete_account_webhook_service(call):
        _LOGGER.debug("Called delete_account_webhook with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            own_id = call.data["own_id"]
            resp = await hass.async_add_executor_job(
            ...
            _LOGGER.info("Delete account webhook response: %s", resp.text)
            await show_result_notification(hass, "xóa webhook", resp)
        except Exception as e:
            _LOGGER.error("Error in delete_account_webhook: %s", e)
            await show_result_notification(hass, "xóa webhook", None, error=e)

    hass.services.async_register(DOMAIN, "delete_account_webhook", async_delete_account_webhook_service, schema=SERVICE_DELETE_ACCOUNT_WEBHOOK_SCHEMA)

    async def async_get_proxies_service(call):
        _LOGGER.debug("Called get_proxies with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            resp = await hass.async_add_executor_job(
                lambda: session.get(f"{zalo_server}/api/proxies")
            )
            _LOGGER.info("Get proxies response: %s", resp.text)
            await show_result_notification(hass, "lấy danh sách proxy", resp)
        except Exception as e:
            _LOGGER.error("Error in get_proxies: %s", e)
    ...
    async def async_add_proxy_service(call):
        _LOGGER.debug("Called add_proxy with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
            ...
            _LOGGER.info("Add proxy response: %s", resp.text)
            await show_result_notification(hass, "thêm proxy", resp)
        except Exception as e:
            _LOGGER.error("Error in add_proxy: %s", e)
    ...
    async def async_remove_proxy_service(call):
        _LOGGER.debug("Called remove_proxy with: %s", call.data)
        try:
            await hass.async_add_executor_job(zalo_login)
            payload = {
                "proxyUrl": call.data["proxy_url"]
            ...
            _LOGGER.info("Remove proxy response: %s", resp.text)
            await show_result_notification(hass, "xóa proxy", resp)
        except Exception as e:
            _LOGGER.error("Error in remove_proxy: %s", e)
